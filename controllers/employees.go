...
func (c *EmployeesController) Post() {
	var v models.EmployeesDataInput
...
func (c *EmployeesController) GetAll() {
	...
	l, err := models.GetAllEmployees(query, fields, sortby, order, offset, limit)
	if err != nil {
		c.Data["json"] = err.Error()
	} else {
		c.Data["json"] = l
...
func (c *EmployeesController) Put() {
	idStr := c.Ctx.Input.Param(":id")
	id, _ := strconv.Atoi(idStr)
	v := models.EmployeesDataInput{}
	if err := json.Unmarshal(c.Ctx.Input.RequestBody, &v); err == nil {
		if err := models.UpdateEmployeesById(id, &v); err == nil {
			c.Data["json"] = "OK"
		} else {
			c.Data["json"] = err.Error()
		}
